...
const extractBonusText = (details) => {
  const match = details.match(/\[[^\]]+\]/);
  return match ? match[0] : details;
};
...
for (const rawLine of lines) {
  ...
  if (line.startsWith("*")) {
    ...
    currentEntry = {
      ...
      name: name.trim(),
      details: extractBonusText(details.trim())
    };
    continue;
  }

  if (!currentEntry) {
    if (currentStage === "Adulthood" && isCategoryLine(line)) {
  ...
  if (line.startsWith("Starting Wealth:")) {
    ...
    currentEntry.feature = currentEntry.feature
      ? `${currentEntry.feature} ${line}`
      : line;
  }
}
...
entries.forEach((entry) => {
  if (entry.category === "Psion" && entry.feature) {
    const suffix = " Automatic Psionic unlocks and +1 Psi Point.";
    if (!entry.feature.includes("Automatic Psionic unlocks")) {
      entry.feature = `${entry.feature}${suffix}`;
    }
  }
});
